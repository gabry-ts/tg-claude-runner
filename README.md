...
- **Self-scheduling from Claude**: any JSON file dropped into `workspace/.schedule/` (`{"cron": "...", "prompt": "...", "once": <bool>, "model": <opt>}`) is registered **live** as a persistent job — no `/schedule` command needed. Jobs survive sessions and bot restarts (backed by `jobs.json`), fire in an isolated Claude turn, and confirm back in chat with their id. Set `"once": true` for one-shot reminders. This is the durable sibling of the in-session cron; use it when you need something to fire even after the session ends.
